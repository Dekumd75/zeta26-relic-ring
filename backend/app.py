...
# ─── Universe Constants (from config — never hardcoded) ───────────────────────
C     = meta['speed_of_light_kms']           # km/s
S     = meta['coordinate_scale_unit_km']      # km per grid unit
L_MAX = meta['max_void_hop_distance_km']      # km — single hop limit
...
def void_distance(p1, p2):
    """
    Equation 1 — Void (vacuum) distance between two planet atmospheres:
...
def tp(planet, from_planet, to_planet):
    """
    Equation 3 — Internal planet crust transit time:
...
def tp_breakdown(planet, from_planet, to_planet):
    ...
    return {
        **tp_info,
        ...
        'eq_fiber': f'(2π × {r} × {s}) / ({N} × {F} × {C}) = {tp_info["fiber_s"]:.6f} s',
        'eq_tower': f'{m} × {DT}ms/1000 = {tp_info["tower_s"]:.6f} s',
...
def enumerate_all_paths(source, destination):
    """DFS to enumerate all simple paths (6-node graph — computationally trivial)."""
    graph  = build_graph(ALL_EDGES)
    result = []
...
def full_transmission(source, destination, message):
    ...
    for i in range(len(path)-1):
        ...
        log.append({'time':ts(),'type':'encode',
                    'msg':f'Payload → <b>Base {target_base}</b> (codex of {p2_id})'})
...
def api_route_analysis():
    ...
    # ── All simple paths + their latencies ──
        ...
        for j in range(i+1, len(plist)):
            ...
            all_pairs.append({
                ...
                **bd,
                ...
                'L_MAX_km': L_MAX,
                'constraint_eq':
                    f"L = √(({p2['x']}-{p1['x']})²+({p2['y']}-{p1['y']})²)×{int(S)}"
                    f" − ({int(p1['radius_km'])}+{int(p1['atmosphere_thickness_km'])})"
                    f" − ({int(p2['radius_km'])}+{int(p2['atmosphere_thickness_km'])})"
                    f" = {bd['L_km']:,.0f} km",
            })
    all_pairs.sort(key=lambda x: x['L_km'])

    # ── Hop-level math breakdown for optimal path ──
    hop_math = []
    if opt_path:
    ...
    if opt_path and feasible_paths:
        if len(feasible_paths) == 1:
            reason = f"Only 1 feasible path exists from {src} to {dst} — all others violate the L_MAX = {int(L_MAX/1e6)}M km void hop constraint."
        else:
            ...
            reason = (f"Dijkstra explored all {len(feasible_paths)} feasible paths. "
                      f"The chosen path {' → '.join(opt_path)} has the minimum total latency "
                      f"({round(opt_lat,4)} s). "
                      f"The next-best path {' → '.join(second['path'])} is {diff} s slower.")
    elif not opt_path:
        reason = f"No feasible path exists — all routes from {src} to {dst} require a void hop exceeding L_MAX = {int(L_MAX/1e6)}M km."
