...
# ─── Constants (all from config — no hardcoding) ──────────────────────────────
...
def ring_segments(entry, exit_t, N):
    """Minimum arc segments between two tower indices on the ring."""
    if entry == exit_t:
        return 0
    cw  = (exit_t - entry) % N
    ccw = (entry - exit_t) % N
...
def tp(planet, from_planet, to_planet):
    ...
    m = (s + 1) if s > 0 else 1      # towers hit along the arc

    arc_km     = (2 * math.pi * r * s) / N
    fiber_time = arc_km / (F * C)    # seconds
    tower_time = m * (DT / 1000.0)   # seconds (DT is in ms)
...
def to_base(n, base):
    """Convert decimal integer n to a string in the given base."""
    if n == 0:
        return "0"
    result = []
    while n > 0:
...
def from_base(s, base):
    """Convert a base-N string back to decimal."""
    result = 0
    for ch in s.upper():
...
def encode_message(text, target_base):
    """Encode each character of text into target_base, return list of strings."""
    return [to_base(ord(ch), target_base) for ch in text]

def decode_message(encoded, source_base):
    """Decode a list of base-N strings back to plain text."""
    return ''.join(chr(from_base(v, source_base)) for v in encoded)

# ─── Graph Builder ────────────────────────────────────────────────────────────

def build_edges():
    """Return list of (p1_id, p2_id, L_km, Tv_s) for all feasible pairs."""
...
def build_graph(edges):
    """Adjacency list respecting current dead planets and dead links."""
...
def dijkstra(source, destination):
    """
    Find the lowest-latency path from source to destination.
    Returns (total_latency, path_list, prev_map).
    """
    ...
    dist[source] = 0
    pq       = [(0.0, source, [source], None)]  # (lat, cur, path, prev)
    visited  = set()
    prev_map = {}
    ...
    while pq:
        ...
        for nbr_id, L, t_v in graph[cur]:
            if nbr_id in visited:
                continue
            # Tp at current planet (ring travel to exit tower toward nbr)
            tp_info     = tp(cur_planet, planets[prev] if prev else None, planets[nbr_id])
...
def full_transmission(source, destination, message):
    """
    Simulate a full packet transmission hop-by-hop.
    ...
    if not path:
        return {
            ...
            'reason':        'No valid path — void distance exceeds limit or node is dead',
            'path':          [],
            'hops':          [],
            'log':           [],
            'total_latency': 0,
    ...
    def ts():
        return datetime.fromtimestamp(now + cumulative,
                                      tz=timezone.utc).strftime('%H:%M:%S.%f')[:-3]
    ...
    for hop_idx in range(len(path) - 1):
        ...
        # ── Internal transit at p1 (ring travel to exit tower) ──
        ...
        log.append({'time': ts(), 'type': 'decode',
                    'msg': f'Decoded Base {target_base} back to ASCII internally'})

        # ── At final destination: add arrival tower processing delay ──
...
def api_universe():
    """Return full universe state: planets, edges, tower positions."""
    planet_list = []
    for pid, p in planets.items():
        N = p['active_towers']
        towers = [{'idx': i,
                   'x': tower_position(p, i)[0],
                   'y': tower_position(p, i)[1],
    ...
    # Include over-limit edges for visualisation (shown as dashed/disabled)
    plist = list(planets.values())
    for i in range(len(plist)):
        for j in range(i + 1, len(plist)):
            p1, p2 = plist[i], plist[j]
...
def api_route():
    """Find the fastest (lowest-latency) route between two planets."""
    data = request.json
    src  = data.get('source')
    dst  = data.get('destination')

    if src not in planets or dst not in planets:
        return jsonify({'error': 'Unknown planet ID'}), 400
    if src == dst:
...
def api_send():
    """Simulate a full message transmission with hop-by-hop encoding."""
    data    = request.json
    src     = data.get('source')
    dst     = data.get('destination')
    ...
    return jsonify(full_transmission(src, dst, message))

@app.route('/api/kill_planet', methods=['POST'])
def api_kill_planet():
    """Mark a planet as dead (failed node)."""
...
def api_kill_link():
    """Sever the link between two planets."""
...
def api_restore():
    """Restore all failed planets and links."""
    dead_planets.clear()
    dead_links.clear()
...
def api_encode():
    """Encode a message payload into a specific numerical base."""
    data    = request.json
    message = data.get('message', '')
    base    = int(data.get('base', 10))
    return jsonify({'encoded': encode_message(message, base), 'base': base})
